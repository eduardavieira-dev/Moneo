...
function renderizarCardLivro(livro) {
  return `<div class="cardLivro" id="cardLivro-${livro.id}">
...
function pegarLivros(catego) {
  fetch("/livros")
    ...
    .then((data) => {
      // Aplica o filtro antes de renderizar, eliminando o if/else com template duplicado.
      // Padrão: separar filtragem de apresentação (princípio da separação de responsabilidades).
      const livrosFiltrados =
        catego === "Todos"
          ? data
          : data.filter((livro) => livro.categoria === catego);

      document.getElementById("container-cards").innerHTML = livrosFiltrados
        .map(renderizarCardLivro)
        .join("");
      ...
      categorias.forEach((item) => {
        item.addEventListener("click", () => {
          ...
          const filtroAtivo = document.querySelector(
            `.btnfiltros[value="${valorCategoria}"]`,
          );
          if (filtroAtivo) {
...
function adicionarComentario(livroId) {
  const comentarioTexto = document.getElementById(
    `textoComentario-${livroId}`,
  ).value;
    ...
    .then((data) => {
      ...
      fetch("/comentarios", {
        ...
        .then((data) => {
          ...
          const containerComentarios = document.getElementById(
  ...
  for (let i = 0; i < comentarios.length; i++) {
    ...
    if (comentario.id === comentarioId) {
      ...
      const textoComentario = document.getElementById(
        `comentarioLivro-${comentario.id}`,
      );
      const btnEditar = document.getElementById(`btnEditar-${comentario.id}`);
      const estrelasContainer = document.getElementById(
        `estrelasEditar-${comentario.id}`,
      );
      const estrelasComentario = document.getElementById(
        `estrelasComentario-${comentario.id}`,
      );
