...
async function preencherInvestimentos() {
  try {
    ...
    investimentosCache = investimentos; // armazena para reutilização em atualizarTaxa()

    selectInvestimento.innerHTML =
...
function atualizarTaxa() {
  ...
  // Utiliza o cache local em vez de realizar uma nova requisição HTTP,
  // eliminando latência desnecessária a cada interação do usuário.
  const investimentoSelecionado = investimentosCache.find(
    (inv) => inv.id == selectedId,
  );
  taxaInput.value = investimentoSelecionado ? investimentoSelecionado.taxa : "";
}
...
btnCalculate.addEventListener("click", async (e) => {
  ...
  let nomeInvestimento =
    selectInvestimento.options[selectInvestimento.selectedIndex].text;
  ...
  result.innerHTML = `
    <div class="card-resultado">
       <div class="titulo-resultado">Valor total:</div>
       <p> R$${total}</p>
    </div>
    <div class="card-resultado">
       <div class="titulo-resultado">Valor investido<br> (sem juros):</div>
       <p> R$${investedValue}</p>
    </div>
    <div class="card-resultado">
       <div class="titulo-resultado">Valor dos juros:</div>
       <p> R$${interest}</p>
    </div>
    `;
