document.addEventListener("DOMContentLoaded", () => {
  ...
  form.addEventListener("submit", (event) => {
    ...
    fetch("http://localhost:3000/cursos", {
      ...
      .then((response) => {
        if (!response.ok)
          throw new Error(`Erro ao cadastrar curso: HTTP ${response.status}`);
  ...
  // preencherCampos() chamada no mesmo listener DOMContentLoaded para evitar
  // dois listeners separados para o mesmo evento (padrão idiomático).
  preencherCampos();
});
